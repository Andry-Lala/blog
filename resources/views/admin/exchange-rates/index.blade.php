...
@section('header')
    <div class="ml-4 flex items-center justify-between">
        <h1 class="text-2xl font-semibold text-gray-900">Gestion des Taux de Change</h1>
    </div>
@endsection
    ...
    <div class="px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white shadow rounded-lg mb-6">
            <div class="px-4 py-5 sm:p-6">
                <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">Taux de Change Actuel</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <form action="{{ route('admin.exchange-rates.update-rate') }}" method="POST" class="space-y-4">
                            @csrf
                            <div>
                                <label for="rate" class="block text-sm font-medium text-gray-600">Nouveau taux (USD → MGA)</label>
                            ...
                            <div>
                                <label for="notes" class="block text-sm font-medium text-gray-600">Notes (optionnel)</label>
        ...
        <div class="bg-white shadow rounded-lg mb-6">
            <div class="px-4 py-5 sm:p-6">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg leading-6 font-medium text-gray-900">Types d'Investissement</h3>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($investmentTypes as $type)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $type->max_amount_usd ? number_format($type->max_amount_usd, 2) : 'Illimité' }}</td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if($type->is_active)
                                            <span class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-50 text-green-600">Actif</span>
                                        @else
                                            <span class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-50 text-gray-600">Inactif</span>
                                        @endif
                                    </td>
        ...
        <div class="bg-white shadow rounded-lg">
            <div class="px-4 py-5 sm:p-6">
                <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">Historique des Taux de Change</h3>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Date d'effet</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Taux</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Modifié par</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($exchangeRates as $rate)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $rate->user?->name ?? 'Système' }}</td>
...
<div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 hidden" id="addTypeModal">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3 text-center">
            <h3 class="text-lg leading-6 font-medium text-gray-900">Ajouter un Type d'Investissement</h3>
        </div>
        <form action="{{ route('admin.exchange-rates.store-type') }}" method="POST" class="mt-5">
            ...
            <div class="px-7 space-y-4">
                ...
                <div>
                    <label for="min_amount_usd" class="block text-sm font-medium text-gray-600">Montant minimum (USD)</label>
                    <input type="number" id="min_amount_usd" name="min_amount_usd" step="0.01" min="0.01" required
                           class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-400 focus:border-blue-400">
                </div>
                <div>
                    <label for="max_amount_usd" class="block text-sm font-medium text-gray-600">Montant maximum (USD)</label>
                ...
                <div>
                    <label for="sort_order" class="block text-sm font-medium text-gray-600">Ordre d'affichage</label>
                ...
                <div class="flex items-center">
                    ...
                    <label for="is_active" class="ml-2 block text-sm text-gray-800">Actif</label>
                </div>
            </div>
...
<div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 hidden" id="editTypeModal">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3 text-center">
            <h3 class="text-lg leading-6 font-medium text-gray-900">Modifier un Type d'Investissement</h3>
        </div>
        <form id="editTypeForm" method="POST" class="mt-5">
            ...
            <div class="px-7 space-y-4">
                <div>
                    <label for="edit_name" class="block text-sm font-medium text-gray-600">Nom</label>
                    <input type="text" id="edit_name" name="name" required
                           class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-400 focus:border-blue-400">
                </div>
                <div>
                    <label for="edit_min_amount_usd" class="block text-sm font-medium text-gray-600">Montant minimum (USD)</label>
                    <input type="number" id="edit_min_amount_usd" name="min_amount_usd" step="0.01" min="0.01" required
                           class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-400 focus:border-blue-400">
                </div>
                <div>
                    <label for="edit_max_amount_usd" class="block text-sm font-medium text-gray-600">Montant maximum (USD)</label>
                    <input type="number" id="edit_max_amount_usd" name="max_amount_usd" step="0.01" min="0.01"
                           class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-400 focus:border-blue-400">
                </div>
                <div>
                    <label for="edit_description" class="block text-sm font-medium text-gray-600">Description</label>
                    <textarea id="edit_description" name="description" rows="3"
                              class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-400 focus:border-blue-400"></textarea>
                </div>
                <div>
                    <label for="edit_sort_order" class="block text-sm font-medium text-gray-600">Ordre d'affichage</label>
                ...
                <div class="flex items-center">
                    <input type="checkbox" id="edit_is_active" name="is_active" value="1"
                           class="h-4 w-4 text-blue-500 focus:ring-blue-400 border-gray-300 rounded">
                    <label for="edit_is_active" class="ml-2 block text-sm text-gray-800">Actif</label>
                </div>
            </div>
            <div class="px-7 py-3 bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse rounded-b-lg">
