...
@section('header')
    <div class="ml-4 flex items-center w-full">
        <div class="flex items-center flex-1 min-w-0">
            <a href="{{ route('investments.index') }}" class="mr-2 sm:mr-4 text-gray-500 hover:text-gray-700 flex-shrink-0">
                <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
                </svg>
            </a>
            <h1 class="text-lg sm:text-2xl font-semibold text-gray-900 truncate">
                <span class="hidden sm:inline">{{ __('messages.investments_of') }} {{ $user->first_name }} {{ $user->last_name }}</span>
                <span class="sm:hidden">{{ $user->first_name }} {{ $user->last_name }}</span>
            </h1>
        </div>
    </div>
@endsection
                                    ...
                                    <div class="flex space-x-2">
                                        ...
                                        @if($investment->status === 'Validé')
                                            <a href="{{ route('investments.invoice', $investment) }}"
                                               target="_blank"
                                               class="text-green-600 hover:text-green-800" title="{{ __('messages.generate_invoice') }}">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
